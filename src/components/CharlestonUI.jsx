  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center' }}>
        <div className="eyebrow" style={{ color: 'rgba(51,48,42,0.45)' }}>
          Charleston Round {charlestonRound}
        </div>
        <div style={{ fontSize: 24, fontFamily: 'Playfair Display, serif', color: '#3C5236', fontWeight: 700, marginTop: 4 }}>
          {label.dir}
        </div>
        <div style={{ fontSize: 13, color: 'rgba(51,48,42,0.6)', fontFamily: 'Nunito', marginTop: 4 }}>
          {label.desc}
        </div>
      </div>
      ...
      {incomingTiles && incomingTiles.length > 0 && (
        <div style={{
          background: 'rgba(95,125,79,0.08)',
          border: '1px solid rgba(95,125,79,0.2)',
          borderRadius: 10,
          padding: '10px 14px',
        }}>
          <div style={{ fontSize: 11, color: '#5F7D4F', fontFamily: 'Nunito', fontWeight: 700, marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
            ↓ Tiles received:
          </div>
      ...
      {/* Selection counter */}
      <div style={{ textAlign: 'center', fontSize: 13, fontFamily: 'Nunito', color: selected.size === 3 ? '#5F7D4F' : 'rgba(51,48,42,0.6)' }}>
        Selected: <strong>{selected.size}</strong>/3
        {canBlindPass && (
          <span style={{ color: 'rgba(51,48,42,0.4)', marginLeft: 8, fontSize: 11 }}>
            (blind pass allowed)
          </span>
        )}
      </div>
      ...
      {/* Pass button */}
      {waitingForAI ? (
        <div style={{ textAlign: 'center', fontSize: 13, color: '#5E92B3', fontFamily: 'Nunito', animation: 'pulse 1s ease-in-out infinite' }}>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 999,
            border: 'none',
            background: selected.size === 3 ? '#5F7D4F' : 'rgba(95,125,79,0.12)',
            color: selected.size === 3 ? 'white' : 'rgba(51,48,42,0.35)',
            fontSize: 15,
            fontWeight: 800,
            ...
            boxShadow: selected.size === 3 ? '0 4px 16px rgba(95,125,79,0.35)' : 'none',
          }}
        >
